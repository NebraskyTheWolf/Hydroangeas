...
public class TemplateManager
{
    ...
    {
        Hydroangeas.getLogger().info("Adding queue:");
        ...
        for (AbstractGameTemplate template : templates)
        {
            ...
            Hydroangeas.getLogger().info("ID: " + template.getId() + " Game: " + template.getGameName() + " Map: " + template.getMapName());
        }
    }
        ...
        try
        {
            ...
            for (File file : files)
            {
                if (file.isFile() && file.getName().endsWith(".json"))
                {
                    try
                    {
                        JsonElement data = new JsonParser().parse(new InputStreamReader(Files.newInputStream(file.toPath()), StandardCharsets.UTF_8));
                        if (data == null)
